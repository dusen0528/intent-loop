...
**긴 코딩 작업에서 사용자 제약을 보존하는 Codex·Claude Code 플러그인입니다.**

“원본 파일은 수정하지 마” 같은 조건을 작업 요약과 별도로 저장하고, 대화가 압축되거나 세션이 재개되면 복원합니다.

**[설치](#설치) · [사용 예시](#사용-예시) · [설명](#설명) · [적용 범위](#적용-범위)**

## 설치

**Node.js 22+ · Python 3.10+ · Linux/macOS · 사용할 호스트의 CLI**가 필요합니다. 사용하는 호스트에 맞는 명령을 실행하세요. 기본값은 사용자 전역 설치입니다.
...
1. Codex에서는 `/hooks`에서 새 훅을 한 번 검토·신뢰합니다.
2. 새 세션을 시작합니다.
3. 프로젝트의 `.gitignore` 또는 기존 전역 Git 제외 파일에 `**/.intent-review/`를 추가합니다. 이 폴더에는 사용자 메시지와 제약 원문이 저장됩니다.

<details>
<summary>설치 상세 · 업데이트 · 제거</summary>
...
대화를 압축하면 “주문 데이터를 정리하는 중”이라는 작업은 남아도, “원본 파일은 수정하지 마”라는 조건은 빠질 수 있습니다. Intent Loop는 이런 조건을 별도로 저장하고 같은 세션에 복원합니다.

| 보존할 내용 | 저장 방식 |
|---|---|
| 작업 목표, 진행 상황, 다음 단계 | 기존 에이전트의 작업 요약 |
| 금지사항, 승인 조건, 선호, 출력 형식 | 별도의 제약 목록 |

1. **새 메시지의 제약을 검토합니다.** 기존 모델이 추가·변경·철회 또는 변경 없음을 제출합니다.
2. **검토 후 작업을 허용합니다.** 훅은 제출 전까지 자신이 관찰하는 새 작업 도구 호출을 차단합니다.
3. **압축·재개 후 복원합니다.** 같은 세션의 제약 목록과 검토 상태를 다시 불러옵니다.

별도 추출 모델이나 새로운 작업 루프는 없습니다. 제약의 의미를 해석하고 다음 행동을 선택하는 일은 기존 모델이 맡습니다.

<details>
<summary>설계 배경과 제약 분류</summary>
...
<details>
<summary>훅의 검토 절차와 상황별 동작</summary>
...
<details>
<summary>플러그인 호환성과 동시 세션</summary>
